...
/**
 * 调用 Python /ai/prescription-check 的客户端。
 * 异常映射：
 * - 401（X-Internal-Key 不一致）→ 系统错误，提示密钥配置问题（开发期排查）。
 * - 503 / 超时 / 连接失败 → AI_AUDIT_UNAVAILABLE，上层据此向前端返回"请人工核对"。
 */
@Slf4j
@Component
@RequiredArgsConstructor
public class PrescriptionAuditClient {

    @Qualifier("aiRestClient")
    private final RestClient aiRestClient;
    private final AiProperties aiProperties;

    public AiPrescriptionCheckResponse check(AiPrescriptionCheckRequest request) {
        try {
            return aiRestClient.post()
                    .uri(aiProperties.getPrescriptionCheckPath())
                    .body(request)
                    .retrieve()
                    .body(AiPrescriptionCheckResponse.class);
            ...
            log.warn("AI处方审核服务返回错误状态 {}：{}", e.getStatusCode(), e.getResponseBodyAsString());
            throw new BusinessException(ErrorCode.AI_AUDIT_UNAVAILABLE);
        } catch (ResourceAccessException e) {
            // 连接拒绝 / 超时
            log.warn("AI处方审核服务不可达：{}", e.getMessage());
            throw new BusinessException(ErrorCode.AI_AUDIT_UNAVAILABLE);
        }
    }
}
