...
-- 功能说明：核心流程表，记录患者每次就诊挂号详情，配置多高频复杂查询索引。
...
-- 功能说明：开立与录入大型医技成像检查（如CT、超声等）结果及状态。
...
-- 功能说明：维护医院提供的检查、检验、处置等项目基础收费与科室字典。
...
-- 功能说明：AI原生模块落库核心表。用于承接并存储由大语言模型对话转换生成的结构化门诊病历字段。
...
-- 功能说明：病历与疾病的多对多关联表，支持对同一病历去重关联索引。
...
-- (13) 疾病表 (disease)
-- 功能说明：存储ICD国际编码以及疾病拼音助记码，方便医生端高频检索秒级回显。
...
-- (14) 处方表 (prescription)
-- 功能说明：记录门诊医生开立的药方明细及用法用量，为AI审核安全用药提供基础支撑。
...
-- (15) 药品信息表 (drug_info)
-- 功能说明：存储全部院内可用药品基本规格、厂家、单价和拼音快速查询助记码。
